...
int main(void)
{
	...
	QWORD sensitivity = 0;
	sensitivity = base_module + m_sensitivity; 
	{
	...
	while (1)
	{
		...
		vec3 muzzle;
		rx_read_process(r5apex, localplayer + m_muzzle, &muzzle, sizeof(vec3)); 
		...
		if (target_entity && IsButtonDown(r5apex, IInputSystem, AIMKEY) && AIMBOT_ENABLED == 1)
		{
			if (rx_read_i32(r5apex, target_entity + m_iHealth) == 0)
				continue;
			if (rx_read_i32(r5apex, target_entity + m_bleedoutState) > 0) 
				continue;
			vec3 enmPos;
			rx_read_process(r5apex, localplayer + m_vecAbsOrigin, &local_position, sizeof(vec3));
			rx_read_process(r5apex, target_entity + m_vecAbsOrigin, &enmPos, sizeof(vec3));
			float distance = ((CalcDistance(local_position, enmPos) / 100) * 2);
